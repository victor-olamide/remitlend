...
const mockSetAbsoluteUserScoresBulk = jest.fn<(scores: Map<string, number>) => Promise<void>>();
...
describe('scoreReconciliationService', () => {
  const originalAutoCorrectEnabled = process.env.SCORE_RECONCILIATION_AUTOCORRECT_ENABLED;
  const originalAutoCorrectThreshold = process.env.SCORE_RECONCILIATION_AUTOCORRECT_THRESHOLD;
  ...
  afterEach(() => {
    ...
    if (originalAutoCorrectThreshold === undefined) {
      ...
      process.env.SCORE_RECONCILIATION_AUTOCORRECT_THRESHOLD = originalAutoCorrectThreshold;
    }
  ...
  it('logs divergences and auto-corrects borrowers above the threshold', async () => {
    ...
    const infoSpy = jest.spyOn(logger, 'info').mockImplementation(() => logger as typeof logger);
