...
describe('GET /api/events/status', () => {
  ...
  it('should return connection counts with valid API key', async () => {
    const response = await request(app).get('/api/events/status').set('x-api-key', VALID_API_KEY);
...
describe('EventStreamService', () => {
  ...
  it('should subscribe and unsubscribe borrower clients', () => {
    ...
    const unsubscribe = eventStreamService.subscribeAddress('testUser', 'testUser', mockRes);
    const counts = eventStreamService.getConnectionCount();
  ...
  it('should broadcast events to borrower clients', () => {
    ...
    const unsubscribe = eventStreamService.subscribeAddress('BORROWER1', 'BORROWER1', mockRes);
  ...
  it('should not broadcast to unrelated borrower clients', () => {
    ...
    const unsubscribe = eventStreamService.subscribeAddress('BORROWER_A', 'BORROWER_A', mockRes);
  ...
  it('should enforce a maximum of three connections per user', () => {
    ...
    const unsubscribers = [
      eventStreamService.subscribeAddress('BORROWER_LIMIT', 'BORROWER_LIMIT', createMockResponse()),
      eventStreamService.subscribeAddress('BORROWER_LIMIT', 'BORROWER_LIMIT', createMockResponse()),
      eventStreamService.subscribeAddress('BORROWER_LIMIT', 'BORROWER_LIMIT', createMockResponse()),
    ];
  ...
  it('should close active SSE connections with a shutdown event', () => {
    ...
    expect(borrowerRes.write).toHaveBeenCalledWith(expect.stringContaining('event: shutdown'));
    expect(adminRes.write).toHaveBeenCalledWith(expect.stringContaining('"type":"shutdown"'));
  ...
  it('should emit replay-compatible SSE event payload from sendEvent', () => {
    ...
    expect(mockRes.write).toHaveBeenCalledTimes(1);
    expect(mockRes.write).toHaveBeenCalledWith(expect.stringContaining('id: evt-99'));
    expect(mockRes.write).toHaveBeenCalledWith(expect.stringContaining('event: loan-event'));
    expect(mockRes.write).toHaveBeenCalledWith(expect.stringContaining('LoanRequested'));
  });
});
