...
describe('updateUserScoresBulk', () => {
  describe('standalone (no client)', () => {
    ...
    it('logs success after updating', async () => {
      ...
      expect(mockLoggerInfo).toHaveBeenCalledWith('Applied bulk user score updates', {
        updatedCount: 1,
      });
    });
    ...
    it('propagates db errors and logs them', async () => {
      ...
      await expect(updateUserScoresBulk(new Map([['user1', 5]]))).rejects.toThrow('db error');
  ...
  describe('with pinned client (inside transaction)', () => {
    it('uses client.query instead of pool query', async () => {
      ...
      await updateUserScoresBulk(new Map([['user1', 10]]), mockClient as unknown as PoolClient);
    ...
    it('propagates errors from client.query', async () => {
      ...
      await expect(
        updateUserScoresBulk(new Map([['user1', 5]]), mockClient as unknown as PoolClient),
